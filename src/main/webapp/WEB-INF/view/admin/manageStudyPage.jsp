...
<center>
						<li>共<i class="blue">${pageInfo.total}</i>条记录，当前显示第<i
							...
							href="${pageContext.request.contextPath}/admin/manageStudyPageFinder?currentPage=1"><button
									class="layui-btn layui-btn-normal  layui-btn-sm">首页</button></a> 
									</c:if> <c:if
								test="${pageInfo.hasPreviousPage && pageInfo.pageNum!=1}">
								<a
									href="${pageContext.request.contextPath}/admin/manageStudyPageFinder?currentPage=${pageInfo.pageNum-1}"><button
								...
								test="${pageInfo.pages !=1&& pageInfo.pageNum!=0}"><a
							href="${pageContext.request.contextPath}/admin/manageStudyPageFinder?currentPage=${pageInfo.pages}"><button>
									末页</button></a></c:if> 
						</li>
					</center>
					<div class="cxbottom">
